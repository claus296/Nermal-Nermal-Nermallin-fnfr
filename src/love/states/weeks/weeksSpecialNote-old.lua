...
return {
					...
					elseif noteVer == "jumpScareNote" then
						...
						if mustHitSection then
							if noteType >= 4 then
								...
								enemyNotesJump[id][c]:animate("on", true)
								...
								boyfriendNotesJump[id][c]:animate("on", true)
						...
						if mustHitSection then
							if noteType >= 4 then
								...
								enemyNotesNerm[id][c]:animate("on", true)
								...
								boyfriendNotesNerm[id][c]:animate("on", true)
	...
	update = function(self, dt)
		...
		if not doingDialogue and not jumpIn then
				...
				if events[i].eventTime <= absMusicTime then
					...
					if week ~= 3 then
						if events[i].mustHitSection then
							mustHit = true
							doingTween = true
							camTimer = Timer.tween(1.25, cam, {x = -boyfriend.x + 100, y = -boyfriend.y + 75}, "out-quad", function() doingTween = false end)
						else
							mustHit = false
							doingTween = true
							camTimer = Timer.tween(1.25, cam, {x = -enemy.x - 100, y = -enemy.y + 75}, "out-quad", function() doingTween = false end)
						end
					end
			...
			if musicThres ~= oldMusicThres and math.fmod(absMusicTime, 120000 / bpm) < 100 then
				if spriteTimers[1] == 0 then
					if girlfriend:getAnimName() ~= "fall" then
