...
return {
	enter = function(self)
		...
		shoot = graphics.newImage(love.graphics.newImage(graphics.imagePath("space")))

		shoot.x, shoot.y = 670, 350
		...
		sounds = {
			...
			gun = {
				load = love.audio.newSource("sounds/gunload.ogg", "static"),
				shoot = love.audio.newSource("sounds/gunshoot.ogg", "static")
			},
			hitsounds = {
	...
	gunShoot = function(self)
		...
		audio.playSound(sounds.gun.load)

		enemy:animate("load", false)

		Timer.after(
			(60 / bpm),
			function()
				audio.playSound(sounds.gun.shoot)
